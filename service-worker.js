...
async function handleMessage(message) {
  ...
  switch (type) {
    ...
    case "ANALYSE_IMAGE": {
      ...
      const state = await getState();
      if (state.mode === "OFF") return { success: true, skipped: true };
...
async function analyseImage(imageUrl, mode) {
  ...
  try {
    ...
    return { verdict: "UNKNOWN", confidence: 0, error: err.message, fallback: true };
  }
}
...
async function updateStats(result) {
  ...
  if (result.verdict === "AI_GENERATED") stats.totalBlocked += 1;
  else if (result.verdict === "SUSPECTED") stats.totalLabelled += 1;
